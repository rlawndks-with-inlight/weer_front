...
const VideoCard = (props) => {
    ...
    return (
        <>
            <Card onClick={() => { navigate(`/video/${props.item.pk}`) }} style={{background:`${props.background?props.background:''}`}}>
                <Img style={{
                    backgroundImage: `url(https://img.youtube.com/vi/${props.item.link}/mqdefault.jpg)`
                }} />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                    <div style={{ padding: '6px 0',minHeight:'3rem' }}>{props.item.title}</div>
                    <div style={{ padding: '6px 0 24px 0', fontSize: `${theme.size.font5}` }}>자세히보기 {'>'}</div>
                </div>
            </Card>
        </>
    )
